...
function doGet(e) {
  try {
    ...
    // لو الطلب وصل بصيغة الأكشنات الموحدة (payload) عبر GET، نمرره لنفس منطق doPost بالضبط
    // (أُضيف لتفادي مشكلة CORS المعروفة مع POST من المتصفح مع تطبيقات ويب Apps Script؛
    // السلوك الأصلي لـ doGet بدون payload يبقى كما هو تماماً لأي استخدام سابق).
    if (params.payload) {
      return json_(routeAction_(parseBody_(e)));
    }
...
function doPost(e) {
  try {
    ...
    return json_(routeAction_(payload));
  } catch (err) {
    return json_({ ok: false, error: publicError_(err) });
  }
}
...
function routeAction_(payload) {
  if (payload.action === 'auth_login') {
    ...
    return { ok: true, user: safeUser_(user), session_token: session.token, expires_at: session.expires_at };
  }

  const session = requireSession_(payload);

  if (payload.action === 'data_sync') {
    return buildDashboardData_(session);
  }
  ...
  if (payload.action === 'daily_update') {
    ...
    return { ok: true, message: 'Update logged and notification sent' };
  }
  ...
  if (payload.action === 'approval_request') {
    ...
    return { ok: true, message: 'Approval request logged', approval: item };
  }
  ...
  if (payload.action === 'approval_update') {
    ...
    return { ok: true, message: 'Approval updated', approval: item };
  }
  ...
  if (payload.action === 'task_assignment') {
    ...
    return { ok: true, message: 'Assignment logged and email sent', assignment: item };
  }

  if (payload.action === 'meeting_record') {
    requireCanEscalate_(session);
    const item = appendMeeting_(withActor_(payload, session));
    return { ok: true, message: 'Meeting logged', meeting: item };
  }
...
function parseBody_(e) {
  // يدعم الآن مصدرين للحمولة: جسم POST (السلوك الأصلي)، أو معامل GET باسم "payload"
  // (أُضيف لتفادي مشكلة CORS المعروفة مع طلبات POST من المتصفح لتطبيقات ويب Apps Script).
