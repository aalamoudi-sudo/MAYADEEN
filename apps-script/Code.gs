...
function appendAuditLog_(payload) {
  ...
  // يُكتب الوقت كنص مُنسّق بتوقيت السعودية (KAG_CONFIG.timezone = Asia/Riyadh) بدلاً من كائن Date الخام،
  // لأن Date الخام يُعرض في الشيت حسب إعداد المنطقة الزمنية الخاص بملف Google Sheet نفسه وليس بالضرورة بتوقيت الرياض.
  // هذا يضمن توحيد التوقيت في كل عمليات Audit Log (تسجيل الدخول، الاجتماعات، الاعتمادات، التكليفات، ...) وليس فقط auth_login.
  const riyadhTimestamp = Utilities.formatDate(new Date(), KAG_CONFIG.timezone, 'yyyy-MM-dd HH:mm:ss');
  sheet.appendRow([
    riyadhTimestamp,
    payload.action || '',
