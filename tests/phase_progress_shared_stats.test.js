...
assert.match(renderBody, /getPhaseDecision/);
assert.match(renderBody, /getPhaseRequiredAction/);
assert.match(renderBody, /getPhaseRemainingTime/);
assert.doesNotMatch(renderBody, /getPhaseCardQuickFacts|👤 المسؤول/);
assert.match(renderBody, /remainingTasks=stat\.total-stat\.done|المتبقي للإغلاق/);
assert.match(renderBody, /phase-quick-facts/);
assert.match(renderBody, /onclick="openPhaseTasks/);
assert.match(renderBody, /phase-detail-toggle/);
