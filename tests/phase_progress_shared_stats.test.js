...
assert.match(renderBody, /phaseUnclassified/);
assert.match(renderBody, /PROJECT_PHASE_UNSPECIFIED/);
...
assert.match(renderBody, /عرض مهام هذه المرحلة/);
assert.match(renderBody, /phaseRequirements/);
assert.match(renderBody, /phaseBlockers/);
assert.match(renderBody, /phaseEvents/);
assert.match(renderBody, /جاهزية الانتقال/);
assert.doesNotMatch(source, /id="phaseTasksBody"|class="card phase-tasks-card"/);
assert.match(source, /لم تُحدد متطلبات انتقال هذه المرحلة بعد/);
