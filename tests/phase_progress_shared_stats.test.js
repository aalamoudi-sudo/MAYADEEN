...
assert.match(renderBody, /آخر خمس مهام في المرحلة/);
assert.match(renderBody, /أقرب ثلاث معالم أو مهام قادمة/);
assert.match(renderBody, /إجمالي المهام/);
assert.match(renderBody, /المهام المكتملة/);
assert.match(renderBody, /قيد التنفيذ/);
assert.match(renderBody, /المهام المتأخرة/);
assert.match(renderBody, /لم تبدأ/);
assert.match(renderBody, /slice\(0,5\)/);
assert.match(renderBody, /slice\(0,3\)/);
assert.match(renderBody, /dateParts\.length/);
assert.match(renderBody, /latest\.length/);
assert.match(renderBody, /upcoming\.length/);
