...
assert.match(renderBody, /phaseDetail/);
assert.match(renderBody, /openPhaseTasks/);
assert.match(source, /عرض جميع مهام المرحلة/);
assert.match(renderBody, /phaseMilestones/);
assert.match(renderBody, /phaseAchievements/);
assert.match(renderBody, /phaseDescriptions/);
assert.match(renderBody, /phaseClosure/);
assert.match(renderBody, /الجاهزية للانتقال/);
assert.match(renderBody, /slice\(0,5\)/);
assert.match(renderBody, /closureCard\.hidden=!closureCriteria\.length/);
assert.match(renderBody, /milestonesCard\.hidden=!important\.length/);
assert.match(renderBody, /achievementsCard\.hidden=!achievements\.length/);
...
assert.doesNotMatch(renderBody, /phase-stat-item|phase-donut|إجمالي المهام|قيد التنفيذ|متأخرة.*لم تبدأ/);
