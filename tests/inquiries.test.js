...
test('سياق الطلب يعيد استخدام مراجع التبويبات ولا يحسب summary في استجابة التفاصيل',()=>{
  const source=fs.readFileSync('apps-script/Inquiries.gs','utf8');
  assert.match(source,/kagInquiryContext && kagInquiryContext\.sheets\[name\]/);
  const detailBranch=source.slice(source.indexOf('if (payload.action === "inquiry_detail")'),source.indexOf('lock = LockService.getScriptLock()',source.indexOf('if (payload.action === "inquiry_detail")')));
  assert.doesNotMatch(detailBranch,/inquirySummary_/);
});
